 ...
 * Camadas 1-20 (Pré): Injeção de Contexto Real, Identificação de Lead e Bloqueio de Alucinação.
 * Camadas 21-40 (Pós): Persistência de Perfil DISC, Normalização de Resposta e Sanitização.
 */
...
export async function enviarMensagemMac(mensagemCliente, whatsapp = 'Geral') {
  try {
    // [PRÉ-PROCESSAMENTO] - Camadas de Orientação
    ...
    // [PÓS-PROCESSAMENTO] - Camada de Persistência e Aprendizado
    // O motor analisa a própria resposta para atualizar o DISC no banco
...
/**
 * Função Auxiliar de Inteligência Comportamental
 */
function detectarPerfilPorTom(texto, perfilAtual) {
  if (texto.length < 50) return 'D'; // Mensagens curtas tendem a Dominância
  if (texto.includes('?') || texto.includes('!')) return 'I'; // Engajamento sugere Influência
