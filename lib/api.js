 ...
 * NÚCLEO LÓGICO DE ATENDIMENTO E ANÁLISE COMPORTAMENTAL
 */
...
export async function enviarMensagemMac(mensagemCliente, whatsapp = 'Geral') {
  try {
    // [PRÉ-PROCESSAMENTO] - BUSCA SINCRONIZADA DE DADOS
    ...
    // [MONTAGEM DO CÉREBRO] - INJEÇÃO DE CONTEXTO REAL
    const promptFinal = `
      ${MAC_SYSTEM_PROMPT}
    ...
    // [EXECUÇÃO] - CHAMADA À CAMADA DE INTELIGÊNCIA CLOUDFLARE
...
/**
 * FUNÇÃO AUXILIAR: DIAGNÓSTICO DE COMPORTAMENTO EM TEMPO REAL
 */
...
export async function testarMac() {
  if (!API_URL) return { erro: "API_URL não configurada" };
